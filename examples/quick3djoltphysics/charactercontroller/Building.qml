...
Node {
    ...
    Body {
        ...
        onBodyEntered: body => { teleportTriggerDelay.start() }
        onBodyExited: body => { teleportTriggerDelay.stop() }
    }
    ...
    Body {
        ...
        shape: BoxShape { extents: Qt.vector3d(100, 100, 100) }
        onBodyEntered: (body) => { door.open = true }
        onBodyExited: (body) => { door.open = false }
    }
