    ...
    View3D {
        ...
        Component.onCompleted: {
            for (var i = 0; i < pyramidHeight; ++i) {
                for (var j = Math.floor(i / 2); j < pyramidHeight - Math.floor((i + 1) / 2); ++j) {
                    for (var k = Math.floor(i / 2); k < pyramidHeight - Math.floor((i + 1) / 2); ++k) {
                        var position = Qt.vector3d(-pyramidHeight + 2.0 * j + (i & 1? 1.0 : 0.0), 1.0 + (2.0 + 0.5) * i, -pyramidHeight + 2.0 * k + (i & 1? 1.0 : 0.0))
